    ...
    <div className="flex flex-col gap-0.5 min-w-0 w-full">
      ...
      {lines.slice(1).map((line, i) => {
        ...
        if (trimmed.startsWith('⊖')) {
          return (
            <div key={i} className="bg-red-500/15 border-l-2 border-red-500/50 px-2 py-0.5 -mx-1">
        ...
        if (trimmed.startsWith('⊕')) {
          return (
            <div key={i} className="bg-emerald-500/15 border-l-2 border-emerald-500/50 px-2 py-0.5 -mx-1">
...
export function VoiceChat() {
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-black text-white relative overflow-hidden">
      {/* Header */}
      <div className="relative z-10 flex flex-col items-center pt-6 pb-3 shrink-0 border-b border-violet-500/10">
        <VoiceWaveform isActive={waveformActive} size={200} getAudioLevel={getAudioLevel} />
        <h1 className="text-xl font-semibold tracking-widest uppercase text-violet-300/90 mt-1">
          Matthews Terminal
        </h1>
      ...
      <div className="relative z-10 flex-1 min-h-0 overflow-y-auto px-5 pb-4" style={{ WebkitOverflowScrolling: 'touch' }}>
        <div className="max-w-2xl mx-auto flex flex-col gap-3 pt-4">
          {messages.length === 0 ? (
                ...
                >
                  {msg.role === 'user' ? (
                    <div className="flex justify-end">
                      <div className="max-w-[85%] bg-violet-500/10 border border-violet-500/20 rounded-sm px-4 py-2.5">
                    ...
                    <div className="flex items-stretch gap-2.5 ml-2">
                      ...
                      <div className="flex flex-col items-center w-3 shrink-0">
                        <div className={cn('w-px flex-1', isPrevTool ? 'bg-violet-500/20' : 'bg-transparent')} />
                        <div className={cn(
                          'w-1.5 h-1.5 shrink-0 transition-all',
                          isLastTool && isProcessing
                            ? 'bg-violet-400 shadow-[0_0_6px_rgba(139,92,246,0.6)]'
                            : 'bg-violet-400/40'
                        )} />
                        <div className={cn('w-px flex-1', isNextTool ? 'bg-violet-500/20' : 'bg-transparent')} />
                      </div>
                      {/* Tool content */}
                      <div className={cn(
                        'flex-1 flex items-start gap-2.5 py-2 px-3 rounded-sm border transition-all',
                        isExpanded
                          ? 'bg-white/[0.04] border-violet-500/20'
                          : 'bg-white/[0.02] border-white/[0.06]'
                      )}>
                        <div className="w-5 h-5 rounded-sm bg-violet-500/15 border border-violet-500/20 flex items-center justify-center shrink-0 mt-0.5">
                    ...
                    <div className="bg-white/[0.03] border border-white/[0.06] rounded-sm px-4 py-3 ml-2">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-5 h-5 rounded-sm bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center">
                          <span className="text-[10px] font-bold">M</span>
                        </div>
                        <span className="text-[11px] font-medium text-white/40 tracking-wide">Matthew</span>
                      </div>
          ...
          {isProcessing && (
            ...
            >
              ...
              <span className="text-xs text-white/30 tracking-wide">Working...</span>
            </motion.div>
          )}
      ...
      {/* Bottom controls */}
      <div className="relative z-50 shrink-0 flex flex-col items-center gap-3 pb-8 pt-4 border-t border-violet-500/10 bg-black">
        ...
        <div className="flex items-center gap-4">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.92 }}
            className={cn(
              'relative flex items-center justify-center w-14 h-14 rounded-sm transition-all duration-500',
              isListening
                ? 'bg-violet-500 shadow-[0_0_30px_rgba(139,92,246,0.4)]'
                : 'bg-white/5 hover:bg-white/10 border border-violet-500/20',
              !supported && 'opacity-30 cursor-not-allowed',
            )}
          >
            {isListening && (
              <span className="absolute inset-0 rounded-sm bg-violet-500/30 animate-ping" style={{ animationDuration: '1.5s' }} />
            )}
          ...
          <AnimatePresence>
            {pendingMessage ? (
              <motion.button
                ...
                onClick={handleSend}
                whileTap={{ scale: 0.92 }}
                className="p-2.5 rounded-sm bg-violet-500 text-white shadow-[0_0_15px_rgba(139,92,246,0.3)]"
              >
